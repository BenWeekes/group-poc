...
function event(store, type, body) { store.events.push({ id: crypto.randomUUID(), type, at: new Date().toISOString(), ...body }); }
...
app.post('/v1/promises-to-pay', async (req, res) => { const id = crypto.randomUUID(); await mutateStore((store) => event(store, 'promise_to_pay', req.body)); return res.json({ id, contact_suppressed_until: req.body.payment_date, confirmation: 'Your proposed payment has been recorded pending official written confirmation.' }); });
app.post('/v1/payment-investigations', async (req, res) => { const id = crypto.randomUUID(); await mutateStore((store) => event(store, 'payment_investigation', { id, ...req.body })); return res.json({ reference: id, next_steps: 'An investigation has been opened. Do not make another payment until you receive the official follow-up.' }); });
app.post('/v1/hardship-cases', async (req, res) => { const id = crypto.randomUUID(); await mutateStore((store) => event(store, 'hardship', { id, ...req.body })); return res.json({ reference: id, payment_requests_suspended: true, safe_next_step: 'Payment discussion is paused and a specialist will review the support options.' }); });
app.post('/v1/disputes', async (req, res) => { const id = crypto.randomUUID(); await mutateStore((store) => event(store, 'dispute', { id, ...req.body })); return res.json({ reference: id, next_steps: 'The dispute has been recorded for review.' }); });
app.post('/v1/contact-preferences', async (req, res) => { await mutateStore((store) => { const account = findAccount(store, req.body); if (account) account.contact_preference = req.body.preference; event(store, 'contact_preference', req.body); }); return res.json({ preference: req.body.preference, suppressed: true, confirmation: 'Your communication preference has been recorded.' }); });
app.post('/v1/official-communications', async (req, res) => { const id = crypto.randomUUID(); await mutateStore((store) => event(store, 'official_communication', { id, ...req.body })); return res.json({ reference: id, delivery_summary: 'An official written follow-up has been queued to the registered contact method.' }); });
app.post('/v1/compliance-incidents', async (req, res) => { const id = crypto.randomUUID(); await mutateStore((store) => event(store, 'compliance_incident', { id, ...req.body })); return res.json({ reference: id, result: 'Safety incident logged.' }); });
