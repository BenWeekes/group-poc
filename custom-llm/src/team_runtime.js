...
const GLOBAL_INTERRUPTS = [
  { agent: 'contact_preference', pattern: /\b(stop (calling|contacting|messages?)|do not contact|don't contact|leave me alone)\b/i },
  { agent: 'hardship_support', pattern: /\b(can't sleep|cannot sleep|humiliated|despair|suicid|kill myself|hurt myself|panic|can't cope)\b/i },
...
export function scopedFunctions(session, secrets) {
  ...
  for (const handoff of agent.handoffs || []) {
    result.push({ type: 'function', function: { name: `handoff_to_${handoff.to}`, description: handoff.description, parameters: handoff.capture || { type: 'object', properties: {} } } });
  }
  for (const candidate of session.llm.agents || []) {
    if (candidate.available_from === '*' && candidate.name !== agent.name) {
      result.push({ type: 'function', function: { name: `handoff_to_${candidate.name}`, description: `Transfer to global agent ${candidate.name}.`, parameters: { type: 'object', properties: { escalation_reason: { type: 'string' } }, required: ['escalation_reason'] } } });
    }
  }
  return result;
}
...
export async function runTeamTurn(session, userText) {
  ...
  for (let pass = 0; pass < MAX_TOOL_PASSES; pass += 1) {
    ...
    const messages = [...systemMessages(session, secrets), ...session.history.slice(-Math.max(1, agent.max_history || 32))];
